...
expected = ("계약단가", "단위", "0 이상", "초과", "전회수량 +", "계약수량 -", "전회금액 +", "계약금액 -", "같은 비용항목", "집계금액", "산출수량 전달", "철근 강종", "호칭지름", "굵은골재 최대치수", "슬럼프", "전용횟수", "복잡도", "설치높이", "수량산정 기준", "직전 번호", "2회차 이상", "기성금액항목이 하나 이상", "집계 원천", "수량산정 기준과 이를 표현", "분류용 상위 클래스", "전회누계 원천은 최대 하나", "동일한 계약항목", "동일한 단위", "비용항목 식별코드", "비용항목명", "전회수량 또는 전회금액", "정확히 하나의 기성서류")
missing = [token for token in expected if not any(token in message for message in messages)]
if missing: raise SystemExit(f"[ERROR] undetected expected violation categories {missing}\n{report}")
...
required_progress_paths = {R.contractQuantity, R.contractUnitPrice, R.contractAmount, R.previousCumulativeQuantity, R.currentQuantity, R.cumulativeQuantity, R.remainingQuantity, R.previousCumulativeAmount, R.currentAmount, R.cumulativeAmount, R.remainingAmount}
reported_paths = set(report_graph.objects(None, SH.resultPath))
if missing_paths := required_progress_paths - reported_paths:
    raise SystemExit(f"[ERROR] missing ProgressPaymentItem values were not detected: {sorted(map(str, missing_paths))}")
print(f"[OK] {len(expected)} expected SHACL violation categories detected")
